...
int read_label_client(string file_name, std::vector<int64_t>& client_ids, std::vector<bool>& labels)
{
    std::ifstream file(file_name); // 打开 JSON 文件
    json json_obj;
    file >> json_obj; // 将 JSON 数据解析为 nlohmann::json 对象
...
int save_csv(std::vector<std::vector<int>> data, string file_name) {
