...
void PirParms::set_seal_parms(uint64_t poly_degree,
  ...
  _seal_parms = seal::EncryptionParameters(seal::scheme_type::bfv);
    // _seal_parms = seal::EncryptionParameters(seal::scheme_type::bgv); //by zhangqizhi.zqz
...
void PirParms::print_seal_parms() {
  ...
  else if (_seal_parms.scheme() == seal::scheme_type::bgv)  //by zhangqizhi.zqz
  {
    scheme_name = "BGV";
  }
  else {
  ...
  /*
    ...
    */
  if (_seal_parms.scheme() == seal::scheme_type::bfv || _seal_parms.scheme() == seal::scheme_type::bgv)  //by zhangqizhi.zqz
  {
...
void PirParms::get_all_index_hash_result(const uint64_t num_payloads,
  ...
  for (uint64_t index = 0; index < num_payloads; index++) {
    ...
    for (auto &position : result) {
      ...
      _hash_index[std::to_string(index * _table_size + position)] =
          _bucket[position].size() - 1;        // _hash_index from _bucket 
    }
  }
  ...
  std::cout<<"_bucket.size()="<<_bucket.size()<<std::endl;
  std::cout<<"_bucket[0].size()="<<_bucket[0].size()<<std::endl;
  std::cout<<"_hash_index.size()="<<_hash_index.size()<<std::endl;
